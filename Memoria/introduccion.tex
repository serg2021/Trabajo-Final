...
En la primera versión se utilizó un framework llamado ``DeepLab'' \cite{deeplab} para realizar el proceso de segmentación semántica con unos resultados muy buenos. Para este proyecto, a través de una exhaustiva búsqueda de modelos y tras muchas pruebas, hemos utilizado uno conocido como ``Swiftnet'' \cite{swiftnet}, el cual conlleva una mejora realmente interesante.

Afortunadamente, ``Swiftnet'' es un modelo de código abierto en GitHub de modo que cualquiera puede utilizarlo a su disposición \cite{github_swiftnet}. Todo lo que hay que hacer es clonar el repositorio de en el que se encuentra utilizando el siguiente comando en la terminal del sistema \cite{clonar}:
...
\begin{enumerate}
	...
	\item Acceso a la librería de Deep Learning llamada ``PyTorch'' \cite{pytorch}
