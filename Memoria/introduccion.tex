...
Para ello usamos una cámara en la parte frontal del vehículo que va tomando imágenes con una cierta frecuencia \ref{fig:ImgOrig}. Gracias a un sistema de \ac{SS}, conseguimos saber qué es cada cosa en cada foto, es decir; qué píxeles son los correspondientes a un vehículo, peatón, semáforo, acera, etc \ref{fig:ImgSegm}. Con esa información se construye un vector de características \ref{fig:Hist} que será empleado por los módulos de inteligencia artificial para realizar una regresión o estimación de la velocidad a la que se debe circular \ref{fig:Regre}. Todo este proceso se puede visualizar en las siguientes figuras:
